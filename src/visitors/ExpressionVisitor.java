...
// update code
	...
	public Expression visitReferenceExpression(ReferenceExpressionContext ctx) {
		if(!documentVisitor.scopes.isEmpty()) {
					Symbol symbol = new Symbol();
					symbol.setName(ctx.getChild(0).getText());
					if(!documentVisitor.referExists.isEmpty())
                   if(documentVisitor.testName(documentVisitor.referExists.peek(),"cp-app")){}
					else
					{

                         symbol.setScope(documentVisitor.scopes.peek().getParent());
                         documentVisitor.scopes.peek().getParent().addSymbol(symbol);

					}
					else
					{
						symbol.setScope(documentVisitor.scopes.peek());
						documentVisitor.scopes.peek().addSymbol(symbol);
					}
				}
